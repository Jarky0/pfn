...
function setupTeamControls() {
  ...
  addTeamBtn.addEventListener("click", function () {
    ...
    console.log(
      `Button geklickt: Team hinzufügen, neue Anzahl: ${newTeamCount}`
    );
  ...
  removeTeamBtn.addEventListener("click", function () {
    ...
    if (teamChips.length > 2) {
      ...
      console.log(
        `Button geklickt: Team entfernen, neue Anzahl: ${newTeamCount}`
      );
      renderTeamChips(newTeamCount);
...
function renderTeamChips(count) {
  ...
  if (
    startGameBtn &&
    window.wordLoader &&
    window.wordLoader.wordPairs &&
    window.wordLoader.wordPairs.length > 0
  ) {
...
function initializeGame() {
  ...
  if (
    ...
    console.error(
      "Error: One or more required scripts failed to load properly"
    );
    ...
    if (fileStatus) {
      fileStatus.textContent =
        "Error loading game resources. Please reload the page.";
  ...
  if (buttons.startGame)
    buttons.startGame.addEventListener("click", () => gameLogic.initGame());
  if (buttons.startRound)
    buttons.startRound.addEventListener("click", () => gameLogic.startRound());
  if (buttons.point)
    buttons.point.addEventListener("click", () => gameLogic.addPoints(1));
  if (buttons.compound)
    buttons.compound.addEventListener("click", () => gameLogic.addPoints(3));
  if (buttons.skip)
    buttons.skip.addEventListener("click", () => gameLogic.addPoints(-1));
  if (buttons.endRound)
    buttons.endRound.addEventListener("click", () =>
      gameLogic.endCurrentRound()
    );
  if (buttons.newGame)
    buttons.newGame.addEventListener("click", () => gameLogic.resetGame());

  // Check if the game start should be enabled
  if (
    wordLoader &&
    wordLoader.wordPairs &&
    wordLoader.wordPairs.length > 0 &&
    buttons.startGame
  ) {
